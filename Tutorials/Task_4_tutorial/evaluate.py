def get_F_score(predictions, ground_truths):
    ...
    num_true = sum((pred == 1 and int(gt) == 1) for pred, gt in zip(predictions, ground_truths))
    num_false_negative = sum((pred == 0 and int(gt) == 1) for pred, gt in zip(predictions, ground_truths))
    num_false_positive = sum((pred == 1 and int(gt) == 0) for pred, gt in zip(predictions, ground_truths))
    ...
    acc = (num_true + sum((pred == 0 and int(gt) == 0) for pred, gt in zip(predictions, ground_truths))) / len(predictions)
